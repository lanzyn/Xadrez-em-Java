...
public class Tabuleiro {
    ...
    public Peça peça(int linha, int coluna) {
        if (!existePosição(new PosiçãoTabuleiro(linha, coluna)))
    ...
    public boolean existePosição(PosiçãoTabuleiro posição) {
        if (posição.getLinha() < 0 || posição.getLinha() >= linhas || posição.getColuna() < 0
                || posição.getColuna() >= colunas) {
            return false;
        } else
            return true;
    }
