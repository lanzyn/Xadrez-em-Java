...
public class PartidaXadrez {
    ...
    public PeçaXadrez substituirPeçaPromovida(String tipo) {
        ...
        PeçaXadrez novaPeça;
        if (tipo.equals("B"))
            novaPeça = new Bispo(tabuleiro, promovido.getCor());
        else if (tipo.equals("C"))
            novaPeça = new Cavalo(tabuleiro, promovido.getCor());
        else if (tipo.equals("Q"))
            novaPeça = new Rainha(tabuleiro, promovido.getCor());
        else
            novaPeça = new Torre(tabuleiro, promovido.getCor());
    ...
    public PeçaXadrez executarMovimento(PosiçãoXadrez origem, PosiçãoXadrez destino) {
        ...
        if (peçaMovida instanceof Peão) {
            if ((peçaMovida.getCor() == Cor.BRANCO && destinoTabuleiro.getLinha() == 0)
                    || (peçaMovida.getCor() == Cor.PRETO && destinoTabuleiro.getLinha() == 7)) {
        ...
        // En passant
        if (peçaMovida instanceof Peão && (destinoTabuleiro.getLinha() == origemTabuleiro.getLinha() - 2
                || destinoTabuleiro.getLinha() == origemTabuleiro.getLinha() + 2)) {
    ...
    public Peça fazerMovimento(PosiçãoTabuleiro origem, PosiçãoTabuleiro destino) {
        ...
        // Roque pequeno
        if (peçaMovida instanceof Rei && destino.getColuna() == origem.getColuna() + 2
                && peçaMovida.getCor() == Cor.BRANCO) {
    ...
    public void validarPosiçãoOrigem(PosiçãoTabuleiro posição) {
        if (!tabuleiro.existePeça(posição))
            throw new ExceçãoXadrez("Não existe peça na posição de origem");
    ...
    public void validarPosiçãoDestino(PosiçãoTabuleiro origem, PosiçãoTabuleiro destino) {
        if (!tabuleiro.peça(origem).movimentoPossivel(destino))
            throw new ExceçãoXadrez("Movimento invalido! A peça escolhida não pode se mover para a posição de destino");
    }
